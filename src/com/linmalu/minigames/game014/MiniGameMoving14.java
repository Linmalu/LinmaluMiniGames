...
public class MiniGameMoving14 implements Runnable
{
	...
	public MiniGameMoving14()
	{
		for(int i = 0; i < data.getPlayerAllCount() * 10; i++)
		{
			...
			Entity entity = data.getMapData().getWorld().spawnEntity(loc, EntityType.SHEEP);
			entity.setSilent(true);
			// sheep.setAI(false);
			entity.setTicksLived(100);
			data.addEntity(entity);
			entitys.put(entity, entity.getLocation().clone());
		}
	...
	public void run()
	{
		if(data.isGame2())
		{
			for(Entity sheep : entitys.keySet())
			{
				if(!sheep.isDead())
				{
					MapData md = data.getMapData();
					if(LinmaluMath.distance(entitys.get(sheep), sheep.getLocation()) > 5 || sheep.getTicksLived() > 100)
					{
						...
						entitys.put(sheep, sheep.getLocation().clone());
					}
